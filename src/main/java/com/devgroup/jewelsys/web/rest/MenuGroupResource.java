...
/**
 * REST controller for managing {@link com.devgroup.jewelsys.domain.MenuGroup}.
 */
@RestController
@RequestMapping("/api")
public class MenuGroupResource {
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/menu-groups/{id}", consumes = "application/merge-patch+json")
    ...
    public ResponseEntity<List<MenuGroupDTO>> getAllMenuGroups(Pageable pageable) {
        log.debug("REST request to get a page of MenuGroups");
        Page<MenuGroupDTO> page = menuGroupService.findAll(pageable);
