...
export class DataCategoryComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.dataCategoryService
      ...
      .subscribe(
        (res: HttpResponse<IDataCategory[]>) => {
  ...
  trackId(index: number, item: IDataCategory): number {
    return item.id!;
  }

  delete(dataCategory: IDataCategory): void {
    const modalRef = this.modalService.open(DataCategoryDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IDataCategory[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
