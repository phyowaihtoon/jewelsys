...
export class RoleMenuMapComponent implements OnInit {
  ...
  protected onSuccess(data: IRoleMenuMap[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/role-menu-map'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
